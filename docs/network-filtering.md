...
The order is load-bearing: each conf lands before the hosts file that steers traffic to its consumer, and `l7-auth-hosts` last, so a torn render (files are not written atomically) is always the fail-closed one — a host retargeted before its conf exists only 403s, it never retargets against a stale conf. Both readers re-stat after reading, never cache a failed parse, and fall to the empty set.
