...
The release gate executes all six package version paths, boots the full VM check, exercises Hermes managed-home initialization and overlay placement, verifies the OMP launcher and its flagless auth-management path, version-checks the opt-in pi package directly, and builds both harness-bearing OCI images from the same source tree.
