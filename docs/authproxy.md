...
`:id` is accepted in **both** forms: a numeric id (`^[0-9]{1,19}$`, ASCII digits only, no sign, no leading zero, compared as i64) or a single segment whose decoded value is a project path. Archive extensions are a **closed set** (`tar.gz`, `tar.bz2`, `tar`, `tar.zst`, `zip`), never a wildcard suffix.

Everything else — `access_tokens`, `variables`, `deploy_keys`, `groups`, `/-/**`, `/uploads`, `/assets`, `raw`, registries, `/oauth/**`, `/admin`, `info/lfs/**`, `gitlab-lfs/**`, `git-upload-archive` — routes **nowhere** (a *named* 403), and `DELETE` is in no capability's method set.

Capability → route:

| cap | routes |
|---|---|
| `git-read` | `git-refs` (`service=git-upload-pack`), `git-upload`, `api-archive` |
...
Only `api-user` and `api-version` are **ambient** (allowed by any issues/mr grant with no scope test — they name no project). `api-project` names one, takes the scope test like every other `:id` route, and is bound to the grant's own project in both id forms — exactly as the legacy tier compiled it (two exact rules). An ambient project lookup would let any issues/mr grant use the owner's token as an enumeration oracle over every project it can see.

Scope tests reuse the existing predicates' semantics: `project` ⇒ the project ref equals the normalized `repo`, or the numeric id equals `project_id`; `namespace` ⇒ the path form is under the **slash-terminated** `prefix` (so `grp-secret` stays excluded), or the numeric id is in `projects[]`; `instance` ⇒ any well-formed project reference. `authenticate` returns `Authorization: Basic base64(git_user:token)` on the three git routes and `Authorization: Bearer <token>` on the API routes — byte-identical to the addon's git-vs-API split, so upstream behaviour does not change.
