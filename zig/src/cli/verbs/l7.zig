// `cogbox l7` - resolve --name to a config/runtime path pair, then hand the
// rest of argv off to the l7 module's dispatch. Mirrors verbs/remap.zig.
...
pub fn run(
	...
	while (i < argv.len) : (i += 1) {
		...
		if (std.mem.eql(u8, a, "--help") or std.mem.eql(u8, a, "-h")) {
			try help.print(io, help.L7);
			return;
		}
		if (std.mem.eql(u8, a, "--name") or std.mem.eql(u8, a, "-n")) {
			i += 1;
			if (i >= argv.len) {
				util.die(allocator, io, "l7", exit_codes.usage, "{s} requires a value", .{a});
			}
			name = argv[i];
			continue;
		}
	...
	if (name) |n| {
		if (std.mem.eql(u8, n, "default")) {
			util.die(allocator, io, "l7", exit_codes.dataerr, "'default' is reserved. Omit --name to use the default instance.", .{});
		}
		if (!parse.isValidName(n)) {
			util.die(allocator, io, "l7", exit_codes.dataerr, "instance name must start with a letter and contain only [a-zA-Z0-9-] (max 64 chars)", .{});
		}
	}
